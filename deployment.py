...
def load_data():
    ...
    if os.path.exists("final_data_with_sentiment.csv"):
        ...
        if pipeline is not None:
            ...
            try:
                if isinstance(pipeline, dict):
                    ...
                    balance_df['sentiment'] = pipeline.predict(cleaned_reviews_bal)
            except Exception:
                st.warning("⚠ Could not add sentiment column to final_data.csv")
        balance_df.to_csv("final_data_with_sentiment.csv", index=False)
    else:
        balance_df = eda_df.copy()
...
# 📊 EDA Page
    ...
    if pos_text.strip():
        st.image(WordCloud(width=600,height=300,background_color="white").generate(pos_text).to_array())
    if neg_text.strip():
        st.image(WordCloud(width=600,height=300,background_color="black",colormap="Reds").generate(neg_text).to_array())
...
# 💡 Sentiment Insights Page
    ...
    if 'sentiment' in eda_df.columns:
        ...
        st.bar_chart(eda_df['sentiment'].value_counts())
